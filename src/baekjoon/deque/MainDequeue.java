...
public class MainDequeue {
        ...
        static int pollFirst() {
                ...
                if(dequeue[back] == 0) return -1;
                int result = dequeue[back];
                dequeue[back] = 0;
                size--;
                return result;
        }
        ...
        static int front() {
                ...
                return dequeue[front] == 0 ? -1 : dequeue[front];
        }

        static int back() {
                // back: 덱의 가장 뒤에 있는 정수를 출력한다. 만약 덱에 들어있는 정수가 없는 경우에는 -1을 출력한다.
                return dequeue[back] == 0 ? -1 : dequeue[back];
        }
}
